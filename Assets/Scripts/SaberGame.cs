...
public class SaberGame : MonoBehaviour {
    ...
    private RhythmManager.TriggerTiming triggerTiming;
    void Start ()
    {
        triggerTiming = pattern.timing;
        RhythmManager.instance.Subscribe(Spawn, triggerTiming, true);
	}
    ...
    private IEnumerator SpawnAndMoveAndDestroy(RhythmPatternEvent e)
    {
        ...
        float y = Mathf.Lerp(7.5f, 6.5f, e.position.y);
        ...
        float offset = RhythmManager.instance.GetOffset();
        float elapsedTime = 0;
        while (elapsedTime <= offset)
        {
            ...
            Vector3 currentPos = targetPos + Vector3.up * y 
            * curve.Evaluate(t) 
    ...
    {
        RhythmManager.instance.Unsubscribe(Spawn, triggerTiming, true);
    }
}
